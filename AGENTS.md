...
- `KANDER_NO_OVERLAY` disables the project `.kander-config.json` lookup entirely, so only the scope configuration is read. Test packages set it in `TestMain` because fixtures run from inside a checkout, where the lookup would otherwise reach the developer's own overlay and merge it into every fixture; the tests that exercise the lookup clear the variable again.
