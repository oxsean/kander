...
- Cards keep optional task group fields and dependency records. When task_groups is off, no task group is created and every request becomes a single card, whatever the number of goals it contains. When on, plan and execute per KANDER-TASK-GROUP-RULES.md; git must be on as well.
...
batch's final target, and `review advance` also requires a planned batch. Both advance entries
carry the new target into the plan in the same transaction, so the plan's recorded target for a
batch is always its current runtime target; the wrap-up evidence binds the base of the first
planned batch and the closed final target of the last batch, which is that recorded target and is
`source_commit` when history was not rewritten. When history was rewritten, the patch comparison
covers base to closed final target and therefore includes every fix round. A board written before
advance synced the plan can still record a stale target: `check`, `move done` and `review progress`
report it with both SHAs, and `review extend-plan` with `sync_targets` aligns the record to the
current runtime target, keeping the previous record in the plan history. Already completed
historical cards without a plan remain readable as legacy-untracked, never as an invented
historical PASS.
